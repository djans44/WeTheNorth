"""Computing the crests of honour.

In app/ rather than scripts/ because the app has to run this: a week of
scores changes who holds nine titles and how far every streak has run, and
the site cannot wait for someone to remember a command. Score entry calls
recompute() when it saves; scripts/award_crests.py is the same code with a
dry run and a printout in front of it.
...
Three shapes, and the difference is when the answer can be known:
  ...
  career   Spans seasons. Every held crest is career-shaped and is recomputed
           on every run, because a week's play can take one off its holder.
           Each is written three ways over: once with no season, which is who
           holds it today; once per finished season, which is who held it when
           that season closed; and once per week played, which is the history
           of how it actually moved.

Auto crests are derived, so writing is a rebuild rather than an append: the
rows in scope are deleted and recomputed. Rows granted by a commissioner --
award_mode 'manual', or any row with awarded_by set -- are never touched.
"""
...
def market_titles(conn, upto=None):
    """Master of Coin and Master of Whisperers, as they stood at a moment.

    `upto` is a season to count through, or None for "as things stand". The
    same shape as held(): compute() calls it once with no cut for the current
    holder, and once per completed season so the title has a history. Without
    that these two had a holder and nothing else -- Josh led the spending
    after 2022 and Matt the dealing after 2023, and the page could not say so.

    No weekly rows. A transaction carries a date and not a week, and a title
    counting a whole career does not move on a Tuesday in any meaningful way.

    They read the transactions rather than summing the season awards:
    counting awards would make one crest depend on another having run first,
    and a rebuild would answer differently depending on the order it ran in.
    """
    out = collections.defaultdict(list)
    have = {r["season_year"] for r in
            q(conn, "select distinct season_year from transactions")}
    ...
    done = [y for y in complete_seasons(conn, every)
            if y in have and (upto is None or y <= upto)]
    if not done:
        return out

    # Money. faab_amount is null for a free agent and for a plain waiver
    # claim, so this sums what was actually bid. A floor, then whoever
    # reached the leading total first -- see COIN_FLOOR.
    ...
    """, (done, COIN_FLOOR)), "spent"), "reached"):
        out["most_faab"].append(
            (r["owner_id"], upto, None, "$%s spent" % _plain(r["spent"]), 0))
    ...
    """, (done, WHISPER_FLOOR)), "n"), "reached"):
        out["most_trades"].append(
            (r["owner_id"], upto, None, "%d trades" % r["n"], 0))
    return out
...
def compute(conn, seasons, weeks_from=1):
    """Every award for the seasons given, as {code: [rows]}.

    `weeks_from` limits the weekly title snapshots to that week onward, which
    is all that entering a week's scores can invalidate: week nine cannot
    change who held a title in week eight. It is what keeps a save under a
    couple of seconds instead of fifteen.
    """
    ...
    # Who holds each title now.
    for code, rows in held(conn).items():
        awards[code] += rows
    for code, rows in market_titles(conn).items():
        awards[code] += rows

    done = set(complete_seasons(conn, seasons))
    for y in seasons:
        # Who held each when the season closed. Only for a season that has
        # one: an in-progress year has no close to record.
        if y in done:
            for code, rows in held(conn, y).items():
                awards[code] += rows
            for code, rows in market_titles(conn, y).items():
                awards[code] += rows
        # And after every week played, playoffs included -- Bane of Their
        # Rival counts every meeting and some of those are playoff games.
        last = q(conn, """
            select max(week) as w from game_log where season_year = %s
        """, (y,))[0]["w"] or 0
        for w in range(max(1, weeks_from), last + 1):
    ...
    for code in list(awards):
        ...
        awards[code] = kept
        dropped += lost
    return awards, dropped


def write(conn, awards, season=None, weeks_from=1):
    """Replace the auto rows this run recomputed. Returns (removed, written).
...
def recompute(conn, season, weeks_from=1):
    ...
    return write(conn, awards, season, weeks_from)
